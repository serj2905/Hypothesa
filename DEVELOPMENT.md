...
- Первый открытый ответ и follow-up хранятся раздельно. В пилоте задаётся не более
  одного нейтрального уточнения с зафиксированной причиной.
- Для prevalence, рейтинга и BERTopic используется только faithful
  `spontaneous_summary`; обогащённая `summary` служит для причин и примеров, поэтому
  новая тема, появившаяся лишь после подсказки, не увеличивает основной рейтинг.
